...
class PlotterXY extends ImagickPlotter
{
    ...
    public function draw(array & $data)
    {
        ...
        foreach ($data as $idx => $generation) {
            ...
            $plotColor = imagecolorallocate($this->handle, (int) $rgb[0], (int) $rgb[1], (int) $rgb[2]);
            // legend
            $step = floor($idx / $curvesCount * 10) / 10;
            if ($lastStep !== $step) {
                imagefttext($this->handle, (int) ($this->height / 50), 0, (int) ($this->width * 0.92), (int) ($this->height * ($step + 1 / 20)), $plotColor, './bin/akukamu.otf', "Curve $idx");
                $lastStep = $step;
            }
            // plotting
            foreach ($generation as $plot) {
                ...
                imagefilledellipse($this->handle, (int) $x, (int) $y, 4, 4, $plotColor);
            }
